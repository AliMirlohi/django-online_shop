...
from .forms import EditProfilModelForm, ChangePasswordForm
...
class EditUserProfilePage(View):
    def get(self, request: HttpRequest):
        ...
        edit_form = EditProfilModelForm(instance=current_user)
        context = {
            'form': edit_form,
    ...
    def post(self, request: HttpRequest):
        current_user = User.objects.filter(id=request.user.id).first()
        edit_form = EditProfilModelForm(request.POST, request.FILES, instance=current_user)
...
class MyShopping(ListView):
    ...
    def get_queryset(self):
        ...
        query_set.filter(user_id =request.user.id, is_paid =True)
        return query_set
@login_required
